...
OUT="official_mobilefacenet/w600k_stage80_we2friendly_relu6_a"
LOG="logs/w600k_stage80_we2friendly_relu6_a.log"
...

{
    ...
    /home/harve/.local/bin/uv run python -u train_w600k_stage96_pair_finetune.py \
        --keep-channels 80 \
        --activation-override suspect-relu6 \
        ...
        --out-dir "${OUT}" \
        --num-calib 500
    cp -f "${OUT}/w600k_stage96_pairft.int8.tflite" "${OUT}/w600k_stage80_we2friendly_relu6_a.int8.tflite"
    if command -v vela >/dev/null 2>&1; then
        vela "${OUT}/w600k_stage80_we2friendly_relu6_a.int8.tflite" \
    ...
    /home/harve/.local/bin/uv run python -u evaluate_embedding_models.py \
        ...
        --model we2_relu6="${OUT}/w600k_stage80_we2friendly_relu6_a.int8.tflite" \
