...
class FakeQuantOps(nn.Module):
    """Per-tensor affine fake-quant using elementary ops (Round/Clamp/Mul/Add) so it
    exports cleanly through onnx2tf. Snaps activations onto the exact QAT int8 grid:
        xq = clamp(round(x/s + z), -128, 127);  x = (xq - z) * s
    TFLite then observes grid-aligned values -> its scale matches the QAT scale."""
    def __init__(self, scale, zp):
        super().__init__()
        self.s = float(scale); self.z = float(zp)

    def forward(self, x):
        xq = torch.clamp(torch.round(x / self.s + self.z), -128, 127)
        return (xq - self.z) * self.s
    ...
    for parent in m.modules():
        for cname, child in list(parent.named_children()):
            if isinstance(child, FakeQuantize):
                ...
                setattr(parent, cname, FakeQuantOps(s, z))
                n += 1
    print(f"replaced {n} FakeQuantize -> FakeQuantOps")
    return m.eval()
