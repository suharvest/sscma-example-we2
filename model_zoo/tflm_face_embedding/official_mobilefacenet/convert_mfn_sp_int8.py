#!/usr/bin/env python3
"""Convert MFN singlepath SavedModel to INT8 TFLite for Ethos-U55 deployment.

The model has in-graph normalization (Sub(127.5) * Mul(1/127.5)),
so calibration images are fed as raw float32 [0, 255] RGB values.
"""
...
SAVED_MODEL_DIR = SCRIPT_DIR / "mfn_sp_saved_model"
CALIB_DIR = REPO_DIR / "calibration_data" / "qat_112"
OUT_PATH = SCRIPT_DIR / "mfn_sp_int8.tflite"
NUM_CALIB = 500
...
def convert():
    ...
    print(f"INT8 output stats: min={int8_output.min()}, max={int8_output.max()}, mean={float(int8_output.mean()):.4f}")

    # Comparison with FP32 TFLite
    float_tflite_path = SAVED_MODEL_DIR / "mfn_s8_v1_singlepath_float32.tflite"
