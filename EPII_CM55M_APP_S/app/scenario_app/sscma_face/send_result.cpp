...
void send_face_recognition_json(
    ...
    /* Mode identifier for ESP32 */
    data_str += "\"mode\": \"face\"";

    /* Image enabled for remote display preview */
    if (jpeg_img && jpeg_img->data && jpeg_img->size > 0) {
        char res_buf[64];
        snprintf(res_buf, sizeof(res_buf), ", \"resolution\": [%d, %d]",
                 jpeg_img->width, jpeg_img->height);
        data_str += res_buf;
        data_str += ", ";
    ...
    if (face_box) {
        ...
        /* Bounding box - use "box" for ESP32 compatibility */
        char bbox_buf[64];
        snprintf(bbox_buf, sizeof(bbox_buf), "\"box\": [%d, %d, %d, %d]",
